...
app.get('/api/v1/record/immunizations', function(req, res) {
    record.getImmunizations('test', function(err, immunizationList) {
        if (err) {
            res.send(400, err);
        } else {
           /* //May be part of model?
            ...
            immunizationJSON.immunizations = immunizationList;
            res.send(immunizationJSON);
        }
    });
});
