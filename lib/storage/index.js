...
app.put('/api/v1/storage/demo', login.checkAuth, function (req, res) {
    ...
    var format = req.body.fileformat;
    var path = req.body.filepath;
    var record_data = {
        path: ''
    };
    if (req.body.fileformat === 'cms') {
        record_data = fs.readFileSync(path).toString();
    } else if (req.body.fileformat === 'xml') {
        record_data.path = path;
    }
    processUpload(req.user.username, record_data, function (err) {
        if (err) {
            res.status(400).send(err);
        } else {
            record.saveEvent('fileUploaded', req.user.username, "User uploaded '" + path, path, function (err) {
                if (err) {
                    res.status(400).send('Event error ' + err);
                } else {
                    res.status(200).end();
