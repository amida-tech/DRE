...
var notificationPort = process.env.WITHINGS_PORT || '3000';
var notificationCallback = 'http://' + notificationUrl + ':' + notificationPort;
...
// Measures
app.get('/api/v1/oauth/withings/weight', getWeightCallbackDaily);

app.get('/api/v1/oauth/withings/weight/all', getWeightCallbackAll);

function getWeightCallbackDaily(req, res) {
    var today = moment().format('YYYY-MM-DD');
    var tomorrow = moment().add(1, 'day').format('YYYY-MM-DD');
    getWeightCallback(req, res, today, tomorrow);
}

function getWeightCallbackAll(req, res) {
    var start = moment('1970-01-01').format('YYYY-MM-DD');
    var end = moment().add(1, 'day').format('YYYY-MM-DD');
    getWeightCallback(req, res, start, end);
}

function getWeightCallback(req, res, start, end) {
    var query = {};
    if (req.user) {
    ...
    WithingsUser.findOne(query, function (err, user) {
        ...
        client.getWeightMeasures(start, end, function (err, data) {
            if (err) {
                return res.send(err);
            }
            ...
            _.forEach(data, function (measure) {
                var vital = {
                    ...
                    "date_time": {
                        "point": {
                            "date": moment(measure.date * 1000).toISOString(),
                            "precision": "day"
                        }
                    },
                    "interpretations": [
...
function weightMeasuresCallback() {}
