...
app.get('/api/v1/oauth/withings/weight', function (req, res) {
    ...
    if (req.user) {
        ...
        query.withingsId = req.query.withingsId;
    } else {
    ...
    WithingsUser.findOne(query, function (err, user) {
        ...
        client.getWeightMeasures(today, tomorrow, function (err, data) {
            ...
            json2csv({
                ...
                fs.writeFile(name, csv, {mode: 777}, function (err) {
                    if (err) {
                        console.log(err);
                        return res.send(err);
                    }
...
app.post('/api/v1/oauth/withings/notification/weight', function (req, res) {
    ...
    request.get({
        url: 'http://devices.amida-demo.com:3000/api/v1/oauth/withings/weight?userid=' + req.query.userid
        ...
        WithingsUser.findOne({
            ...
            if (err) {
                ...
                importRecord(user.username, meta, response.json, function (err, import_results) {
                    if (err) {
                        console.log(err);
                        return res.sendStatus(400);
                    }
