...
function lastLogin2(){
	var loginQuery= Event.find({'event_type': 'loggedIn'}).sort({date: -1});
...
function lastUpdate2(){
	var updateQuery = Event.find({'event_type': 'fileUploaded'}).sort({date: -1});
	updateQuery.exec(function(err,updates){
		if(err){
			...
			if (updates){
				...
				var lastUpdate = Event.findOne({event_type: 'initAccount'}, function(err, update){
					if (err) { 
						console.log(err);
						return
					}
