...
function drugFormList(drugs, callback) {
    var newDrugs = drugs;
    newDrugs.compiledDrugList = [];
    newDrugs.compiledDrugFormGroupList = [];
    async.series([
        function (cb) {
            console.log("compiling drug list");
            if (drugs.drugGroup != null) {
                if (drugs.drugGroup.conceptGroup != null) {
                    async.each(drugs.drugGroup.conceptGroup, function (conceptGroup, cb2) {
                        if (conceptGroup.conceptProperties != null) {
                            async.each(conceptGroup.conceptProperties, function (drug, cb3) {
                                newDrugs.compiledDrugList.push(drug);
                                cb3();
                            }, function (err) {
                                cb2();
                            });
        ...
        function (cb) {
            console.log("compiling drug form groups");
            if (drugs.drugGroup != null) {
                if (drugs.drugGroup.conceptGroup != null) {
                    async.each(drugs.drugGroup.conceptGroup, function (conceptGroup, cb2) {
                        if (conceptGroup.conceptProperties != null) {
                                ...
                                async.series([function (cb4) {
                                    ...
                                    for (var i = 0; i <= possibleFormList.length; i++) {
                                        if (i === possibleFormList.length) {
                                            ...
                                            if (newDrugs.compiledDrugFormGroupList.indexOf(possibleFormList[i]) == -1) {
                                                newDrugs.compiledDrugFormGroupList.push(possibleFormList[i]);
                                            }
                                        }
                                    }
                                })
        ...
        function (cb) {
            newDrugs.compiled = newDrugs.compiledDrugList;
            drugs.dfg = newDrugs.compiledDrugFormGroupList;
            cb();
        }
    ], function (err, results) {
