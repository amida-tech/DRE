...
function drugFormList(drugs, callback) {
    ...
    async.series([
        function (cb) {
            if (newDrugs.drugGroup != null) {
                if (newDrugs.drugGroup.conceptGroup != null) {
                    ...
                    async.series([function (cb4) {
                        ...
                        for (var i = 0; i <= possibleDrugFormList.length; i++) {
                            if (i === possibleDrugFormList.length) {
                                ...
                                if (drug.dfg.indexOf(possibleDrugFormList[i]) != -1) {
                                    drug.dfg.push(possibleDrugFormList[i]);
                                }
                            }
                        }
                    }], function (err, results) {
