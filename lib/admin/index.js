...
app.post('/api/v1/admin/register', function (req, res) {
...
app.post('/api/v1/admin/login', passport.authenticate('local'), function (req, res) {
    res.status(200).end();
});

//logout
app.post('/api/v1/admin/logout', checkAuth, function (req, res) {
    req.logout();
    res.status(200).end();
});

// authentication
app.get('/api/v1/admin/account', function (req, res) {
    res.status(200).send({
...
// get all clients
app.get('/api/v1/admin/clients/all', checkAuth, roles.can('access admin page'), function (req, res) {
    oauth2.clients.findAll(function (err, clients) {
        res.status(200).send(clients);
    });
});

// register a new client
app.post('/api/v1/admin/clients/add', checkAuth, roles.can('access admin page'), oauth2.register);

// delete a client
app.post('/api/v1/admin/clients/delete', checkAuth, roles.can('access admin page'), function (req, res) {
    oauth2.clients.deleteByName(req.body.client_name, function (err) {
        res.status(200).end();
    });
});

// edit approval status
app.post('/api/v1/admin/clients/approve', checkAuth, roles.can('access admin page'), oauth2.approveClient);
