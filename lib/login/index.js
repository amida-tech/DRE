...
passport.use('clientBasic', new BasicStrategy(
  function(clientId, clientSecret, done) {
    ...
    oauth2.clients.findByClientId(clientId, function(err, client) {
      ...
      if (client.tokenEndpointAuthMethod !== 'none' && client.clientSecret !== clientSecret) { return done(null, false); }
...
app.get('/oauth2/login', function(req,res) {
    console.log("body: "+JSON.stringify(JSON.parse(req.body),null,4));
    ...
    if (queryKeys.length > 0) {
        ...
        for (var i = 0; i < queryKeys.length; i++) {
            paramString += paramAnd + queryKeys[i] + "+" + req.query[queryKeys[i]];
            paramAnd = "&";
        }
    }
    if(req.body.redirectUri) {
        //res.redirect(req.body.redirectUri);
        res.redirect('/#/oauth2'+paramString);
    } else {
        res.status(200).end();
    }
});
...
/*
app.post('/oauth2/loggedin', checkAuth, function(req,res) {
    console.log("body: "+JSON.stringify(req.body,null,4));
    console.log("query: "+JSON.stringify(req.query,null,4));
    console.log("data: "+JSON.stringify(req.data,null,4));
    if(req.body.redirectUri) {
        //res.redirect(req.body.redirectUri);
        res.redirect('/oauth2/test');
    } else {
        res.status(200).end();
    }
});
...
/*
app.post('/oauth2/client', checkAuth, function(req,res) {
    oauth2.getClient(req.body.clientId, function(err,client){
        if (err) {
