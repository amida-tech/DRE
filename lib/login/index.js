...
var accountHistory = require('../account-history/index');
...
app.post('/api/v1/register', function(req, res) {
    ...
    newAccount(req.body.username, req.body.password, function(err, account) {
        if (err) {
            ...
            console.log(account);
            accountHistory.saveEvent('initAccount', req.user.username, function(err){
                if(err){
...
app.post('/api/v1/login',
    passport.authenticate('local'), function(req, res) {
        accountHistory.saveEvent('loggedIn', req.user.username, req.ip, function(err){
            if(err){
                res.status(400).send('Event error ' + err);
            } 
        });
...
app.post('/api/v1/logout', function(req, res) {
    req.logout();
    accountHistory.saveEvent('loggedOut', req.user.username, req.ip, function(err){
        if(err){
            res.status(400).send('Event error ' + err);
        } 
    });
