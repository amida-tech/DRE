...
var accountHistory = require('../account-history/utils.js');
...
app.post('/api/v1/register', function(req, res) {
    ...
    newAccount(req.body.username, req.body.password, function(err, account) {
        if (err) {
            ...
            //need to send req.message to UI
            //console.log(res);
            console.log(account);
            res.status(400).send('message');
        } else {
            //console.log(account);
            accountHistory.saveEvent('initAccount', req.body.username, req.ip, null, function(err){
                if(err){
...
app.post('/api/v1/login',
    passport.authenticate('local'), function(req, res) {
        accountHistory.saveEvent('loggedIn', req.user.username, req.ip, null, function(err){
            if(err){
                res.status(400).send('Event error ' + err);
            } 
        });
...
app.post('/api/v1/logout', function(req, res) {
    req.logout();
    accountHistory.saveEvent('loggedOut', req.user.username, req.ip, null, function(err){
        if(err){
            res.status(400).send('Event error ' + err);
        } 
    });
