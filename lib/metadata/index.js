    ...
    function (req, res) {
        ...
        record.getSection('demographics', user, function (err, savedObj) {
            if (err) {
                ...
                // Fix the structure
                var final = [];
                var i, len = entry.entry.length;
                for (i = 0; i < len; i++) {
                    final.push({
                        id: entry.entry[i].resource.id,
                        content: {
                            resourceType: entry.entry[i].resource.resourceType,
                            name: entry.entry[i].resource.name,
                            gender: entry.entry[i].resource.gender,
                            birthDate: entry.entry[i].resource.birthDate,
                            identifier: [{
                                value: user
                            }]
                        }
