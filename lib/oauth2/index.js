...
var clients = module.exports.clients = (function () {
    ...
    var testClients = [{
            ...
            approved: true,
            ownerEmail: 'admin@amida-demo.com'
        }, {
    ...
    return {
        ...
        findByOwnerEmail: function (email, done) {
            ClientModel.where({
                ownerEmail: email
            }).find(
                function (err, clients) {
        ...
        save: function (client_name, client_id, client_secret, launch_uri, redirect_uri, logo_uri, owner_email, token_endpoint_auth_method, grant_types, scope, approved, done) {
        ...
        update: function (client_name, approval_status, done) {
            ClientModel.update({
                'name': client_name
            }, {
                $set: {
...
module.exports.login = [
    passport.authenticate('local', {
        failureRedirect: '/oauth2/login?error=failure'
    }),
    function (req, res) {
...
module.exports.register = function (req, res, next) {
    clients.findByClientName(req.body.client_name, function (err, client) {
        ...
        clients.save(req.body.client_name, client_id, client_secret, launch_uri, redirect_uri, logo_uri, owner_email, token_endpoint_auth_method, grant_types, scope, approved,
            function (err, client) {
                if (err) {
                    res.status(400).send({
                        error: err
...
module.exports.approveClient = function (req, res, next) {
    clients.update(req.body.client_name, req.body.approval, function (err, client) {
        if (err) {
            res.status(400).send({
                error: err
            });
