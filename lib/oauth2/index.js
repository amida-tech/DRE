...
var url = require('url');
var debug = require('debug')('oauth impl');
...
var AuthorizationCode = new Schema({
    ...
    expiresAt: {
        type: Date,
        expires: '5m' // Everythig older is stale and has to be disregarded
    }
});
...
module.exports.authorization = [
    ...
    function (req, res, next) {
        if (req.user) {
            return next();
        } else res.redirect('/oauth2/login?redirectUri=' + encodeURIComponent(req.url) + "&clientId=" + encodeURIComponent(req.query.client_id) + "&responseType=" + encodeURIComponent(req.query.response_type));
    },
        ...
        clients.findByClientId(clientId, function (err, client) {
            ...
            for (i = 0; i < len; i++) {
                ...
                if (url1.protocol === url2.protocol &&
                    ...
                    console.log(redirectUri, redirects[i]);
                    return done(null, client, redirectUri);
                }
            }
            return done('Redirect uri doesn\'t match registered redirects', null);
        });
    }),
