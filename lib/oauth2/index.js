...
server.exchange(oauth2orize.exchange.code(function (client, code, redirectUri, done) {
    authorizationCodes.find(code, function (err, authCode) {
        ...
        authorizationCodes.delete(code, function (err) {
            ...
            accessTokens.save(tokenHash, expirationDate, authCode.userId, authCode.clientId, authCode.scope, function (err) {
                if (err) {
                    return done(err);
                }
...
module.exports.register = function (req, res, next){
        clients.findByClientName( req.body.client_name, function(err, client) {
            ...
            if(client) return res.status(400).send({error: client.clientName +  ' already registered!'});
