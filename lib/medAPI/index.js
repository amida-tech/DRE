    ...
    for (var i = 0; i < med1.length; i++)
        if (med1[i].product) {
            ...
            if (med1[i].product.product && med1[i].product.product.name) {
                medname = med1[i].product.product.name;
                console.log(medname);
                request('https://api.fda.gov/drug/label.json?search=openfda.generic_name:"' + medname + '"+brand_name:"' + medname + '"', function (error, response, body) {
                    if (!error) {
                        console.log(response.body);
                        fs.appendFile('fdaResults.json', response.body, function (err) {
                            if (err) throw err;
...
function queryRxImageCode (req, res) {
	var medcode = req.params.rxcui;
	console.log(medcode);
...
function queryRxNorm(req, res) {
	var medname = req.params.medname;
	console.log(medname);
...
app.get('/api/v1/openfda/:rxcui', login.checkAuth, function (req, res) {
	queryRxImageCode(req, res);
});

app.get('/api/v1/rxnorm/:name', login.checkAuth, function (req, res) {
	queryRxNorm(req, res);
});
