...
app.get('/api/v1/master_health_record/:format?', login.checkAuth, function(req, res) {
    ...
    getMHR(req.user.username, function(err, result) {
        ...
        if (err) {
            ...
            if (format === "json") {
                //return BB JSON
                accountHistory.saveEvent('fileDownloaded', req.user.username, 'MHR - JSON format', function(err){
                    if(err){
                ...
                accountHistory.saveEvent('fileDownloaded', req.user.username, 'MHR - XML format', function(err){
                    if(err){
                        res.status(400).send('Event error ' + err);
                    } 
                });
