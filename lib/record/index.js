...
app.post('/api/v1/record/demographics', login.checkAuth, function(req, res) {
    ...
    // function updateProf(inputInfo) {
        var info = { 'dob': req.body.dob };
        console.log(info, req.user.username, req.body.id);
        record.updateEntry('demographics', req.user.username, req.body.id, null, info, function(err) {
            if (err) {
                throw err;
