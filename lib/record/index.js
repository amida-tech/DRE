...
var historyRecord = require('../../../blue-button-record/index.js');
...
app.get('/api/v1/master_health_record/:format?', login.checkAuth, function(req, res) {
    ...
    getMHR(req.user.username, function(err, result) {
        ...
        if (err) {
            ...
            if (format === "json") {
                //return BB JSON
                historyRecord.saveEvent('fileDownloaded', req.user.username, 'MHR - JSON format', null, function(err){
                    if(err){
                ...
                historyRecord.saveEvent('fileDownloaded', req.user.username, 'MHR - XML format', null, function(err){
                    if(err){
                        res.status(400).send('Event error ' + err);
                    } 
                });
