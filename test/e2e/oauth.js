            ...
            api.get('/oauth2/cleantest').end(function (err, res) {
                ...
                api.post('/oauth2/register').send(temp).expect(200).end(function (err, res) {
                    ...
                    api.get('/oauth2/authorize?redirect_uri=' + encodeURIComponent(callback + '?client_id=' + client_id) + '&response_type=code&client_id=' + client_id).expect(302).end(function (err, res) {
                        ...
                        api.post(login_uri).type('form').send({ username: username, password: password, redirectUri: query.redirectUri }).expect(302).end(function (err, res) {
                            if (err) return done(err);
                            ...
                            api.get(res.header.location).expect(200).end(function (err, res) {
                               ...
                               // Post data to a 'decision' form
                                api.post(decide_uri).type('form').send({ transaction_id: transaction_id }).expect(302).end(function (err, res) {
                                    if (err) return done(err);
                                    ...
                                    api.post('/oauth2/token').type('form').auth(client_id, client_secret).send({  code: query.code, redirect_uri: client.redirect_uris[0], client_id: client_id, client_secret: client_secret, grant_type: 'authorization_code' }).expect(302).end(function (err, res) {
                                        ...
                                        // Test token refresh
                                        api.post('/oauth2/refresh').type('form').auth(client_id, client_secret).send({refresh_token : access_params.refresh_token , client_id : client_id , grant_type : 'refresh_token'}).expect(200).end(function (err, res) {
